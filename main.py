...
def adaptive_window(all_events):
    """
    Compute adaptive context window for all uploaded files combined.

    Rules:
    - >60% gaps ≤ 20 minutes → ±30 minutes
    - >60% gaps >20 min and ≤ 10 hours → ±60 minutes
    - >60% gaps >10 hours and ≤ 24 hours → ±15 hours
    - >60% gaps >24 hours → ±36 hours
    """
    # Flatten all timestamps
    times = sorted([e["ts"] for e in all_events if e.get("ts")])
    if len(times) < 2:
        return 3600, "Not enough events → using default ±60 minutes"

    # Compute gaps in seconds
    diffs = [(times[i+1] - times[i]).total_seconds() for i in range(len(times)-1)]
    if not diffs:
        return 3600, "No gaps found → using default ±60 minutes"

    total_gaps = len(diffs)

    # Count gaps within thresholds
    gap_20min = sum(1 for d in diffs if d <= 20*60)                   # ≤ 20 minutes
    gap_10h   = sum(1 for d in diffs if 20*60 < d <= 10*3600)         # >20 min ≤ 10 hours
    gap_24h   = sum(1 for d in diffs if 10*3600 < d <= 24*3600)       # >10 hours ≤ 24 hours
    gap_more_24h = sum(1 for d in diffs if d > 24*3600)               # >24 hours

    # Determine adaptive window
    if gap_20min / total_gaps > 0.6:
        return 1800, "More than 60% of gaps ≤ 20 minutes → using ±30 minutes"
    elif gap_10h / total_gaps > 0.6:
        return 3600, "More than 60% of gaps >20 min and ≤ 10 hours → using ±60 minutes"
    elif gap_24h / total_gaps > 0.6:
        return 54000, "More than 60% of gaps >10 hours and ≤ 24 hours → using ±15 hours"
    elif gap_more_24h / total_gaps > 0.6:
        return 129600, "More than 60% of gaps > 24 hours → using ±36 hours"
    else:
        return 3600, "No dominant gap pattern → using default ±60 minutes"
...
def recreate_cluster_collection(dim: int):
    ...
    if not client.collection_exists(CLUSTER_COLLECTION):
        client.create_collection(
...
/* Right arrow on first tab only (optional) */
...
with app_tab:
    ...
    if should_index_now and uploaded_files:
        ...
        if all_events:
            # Compute adaptive window considering all uploaded events
            window_sec, reason = adaptive_window(all_events)
            st.session_state.adaptive_window = window_sec
            st.session_state.adaptive_window_reason = reason
            st.info(f"⏱ Adaptive context window selected: {reason}")

            st.session_state.first_upload_done = True
            st.session_state.upload_sig = current_sig
            st.session_state.index_ready = True
        else:
            st.session_state.adaptive_window = 3600
            st.session_state.adaptive_window_reason = "Default ±60 minutes"
        ...
        if not q:
            ...
            if not hits:
                ...
                with st.expander("📑 Search Results (expand/collapse)", expanded=True):
                    ...
                    # Compute adaptive window and get reason
                    # Use the adaptive window calculated from all uploaded files
                    window_sec = st.session_state.get("adaptive_window", 3600)
                    reason = st.session_state.get("adaptive_window_reason", "Default ±60 minutes")

                    # Convert window to human-readable
                    if window_sec < 3600:
                        window_str = f"±{window_sec // 60} minutes"
                    elif window_sec < 86400:
                        window_str = f"±{window_sec / 3600:.1f} hours"
                    else:
                        window_str = f"±{window_sec / 3600:.0f} hours"

                    st.info(f"⏱ Adaptive context window: {window_str} — {reason}")
                    ...
                    if deadlock_hit:
                        ...
                        ctx = fetch_pid_window(dl_ts, pids, window_seconds=window_sec)
                        render_deadlock_report(deadlock_hit, ctx)
                    else:
                        top = hits[0]
                        ctx = expand_context_around_hit(top, window_seconds=window_sec)

                        st.markdown("### 🎯 Top Match")
                        st.code(top.payload.get("full_text", ""), language="text")

                        st.markdown(f"### 🧭 Context ({window_str})")
                        for p in ctx:
    ...
    if st.session_state.get("last_hits"):
        ...
        ">
            ...
            style="
                    background-color:#cce5ff;  /* Same blue as tabs */
